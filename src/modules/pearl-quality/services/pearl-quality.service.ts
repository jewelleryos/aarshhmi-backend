...
export const pearlQualityService = {
  ...
  async list(): Promise<PearlQualityListResponse> {
  ...
  // Get single pearl quality by ID (must belong to PEARLS group)
  async getById(id: string): Promise<PearlQuality> {
    const result = await db.query(
      `SELECT id, stone_group_id, name, slug, description, image_url, image_alt_text, status, metadata, created_at, updated_at
       FROM stone_qualities
  ...
  // Create pearl quality (automatically sets PEARLS stone_group_id)
  async create(data: CreatePearlQualityRequest): Promise<PearlQuality> {
    ...
    if (existingSlug.rows.length > 0) {
      throw new AppError(pearlQualityMessages.SLUG_EXISTS, HTTP_STATUS.CONFLICT)
    }
    ...
    const pearlQuality = result.rows[0]

    // Create system tag for this pearl quality
    await db.query(
      `INSERT INTO tags (tag_group_id, name, slug, source_id, is_system_generated, is_filterable, status)
       VALUES ($1, $2, $3, $4, TRUE, TRUE, TRUE)`,
      [SYSTEM_TAG_GROUPS.PEARL_QUALITY, pearlQuality.name, pearlQuality.slug, pearlQuality.id]
    )

    return pearlQuality
  },

  // Update pearl quality (must belong to PEARLS group)
  async update(id: string, data: UpdatePearlQualityRequest): Promise<PearlQuality> {
    // Check if pearl quality exists and belongs to PEARLS group
    ...
    if (data.name !== undefined || data.slug !== undefined) {
      ...
      await db.query(
        ...
        [updated.name, updated.slug, id, SYSTEM_TAG_GROUPS.PEARL_QUALITY]
      )
    }
  ...
  // Get pearl qualities for product dropdown (with slug)
